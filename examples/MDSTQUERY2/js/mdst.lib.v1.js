...
var mdst = {
    defaultOptions: {
        domainName: "", // if it's not local       
        localPath: "/MDSTWebClientNG/MODULES/MDSTQUERY/",
        workspace: "",
        viewURI:"",
        sourceURI:"",
        onUpdate: function() {
    ...
    $.fn.mdst = function(options) { //
        ...
        function entryPoint() {
            ...
            if (options.workspace === "") {
                console.log("init: " + options.domainName);
                init();
            }
            else {
                console.log("pas init: " + options.domainName);
                $.ajax({
        ...
        function apply(workspace) {
            ...
            $manager.deleteCriterionButtonClass.off("click").on("click", deleteCriterionAction);
        }
        ...
        function discreteCriterionTreatments(workspace, criterion, autocompleteSelectionData) {
            ...
            //VARIABLES DOM
            var $local = {
                criterionElementsSelectedButton: findByMdstId("span", "" + id + "-criterionElementsSelected-button"),
...
//  	$('#viewOption-modal').css('opacity', 1);
            ...
            $local.elementsList.bind("mousedown", function(e) {
                ...
                selected: function(event, ui) {
                    ...
                    console.log($(ui.selected).attr("uri"));
            ...
            $local.requestCriterionCheckallLink
                    .on("click", function(event) {
                        console.log("tata");
                        discreteCriterionCheckall(id, $local.elementsList);
                        $local.popupSelect.multiSelect('select_all');
                    });
            $local.requestCriterionUncheckLink
                    .on("click", function(event) {
                        discreteCriterionUncheck(id, $local.elementsList);
                        $local.popupSelect.multiSelect('deselect_all');
                    });
        ...
        function temporalCriterionTreatments(workspace, criterion, startDateValue, endDateValue) {
            ...
            console.log("timePickerFullFormatStartDate:" + timePickerFullFormatStartDate);
            var timePickerFullFormatEndDate = DateToFormatPickerConverter(endDate);

            var datePickerStart = findByMdstId("input", id + '-datePickerStart');
            var datePickerEnd = findByMdstId("input", id + '-datePickerEnd');
            var criterionElementsSelectedButton = findByMdstId("span", id + "-criterionElementsSelected-button");
            ...
            datePickerStart.datetimepicker({
                ...
                console.log("ev.date.toISOString():" + newISOStartDate);
            ...
            datePickerEnd.datetimepicker({
                ...
                console.log("ev.date.toISOString():" + newISOEndDate);
...
//                $('#bibouille').css('margin-top', newHeight + 20);
        ...
        function sessionDestroyedError(data) {
            ...
            if (data === "false") {
                console.log("'false', the json response");
